  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(p => `
              <li>
                ...
                <button class="remove-participant" data-activity="${name}" data-email="${p}" title="Remove participant">🗑️</button>
              </li>`).join("")}</ul>`
        ...
        // Attach listeners to remove buttons for this activity card
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        // Refresh activities to update UI immediately so new participant appears
